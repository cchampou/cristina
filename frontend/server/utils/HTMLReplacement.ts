...
export function HTMLReplacement(template: string, serverHtml: string, path: string, lang: string) {
  ...
  const htmlWithAlternate = htmlWithLang.replace('<!--alternate-->', supportedLocales.filter(lng => lng !== lang).map(locale => `<link rel="alternate" hreflang="${locale}" href="https://cristinacoellen.com${path}?lang=${locale}">`).join(''));
